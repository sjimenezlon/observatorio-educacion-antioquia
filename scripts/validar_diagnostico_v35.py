...
def main() -> None:
    ...
    checks = [
        check("registry_scale", sources["summary"]["sources"] == 33 and sources["summary"]["indicators"] == 67, "33 fuentes y 67 indicadores"),
        check("registry_version", sources["meta"]["version"] == "V35" and sources["meta"]["research_cut"] == "2026-07-17", "versión y corte sincronizados"),
        check("evidence_distribution", sources["summary"]["evidence"] == {"derivada": 7, "directa": 50, "reportada": 10}, "tipos de evidencia conservan su clasificación"),
        ...
        check("about_section", all(token in html for token in ('id="que-es-materia"', "materia-gris-explicada.webp", "Qué es MaterIA Gris", "Explorar las 67 cifras")), "imagen y explicación integradas a la interfaz"),
        ...
        check("audit_contract", audit["fuentes_v35"]["fuentes_revisadas"] == 33 and audit["diagnostico_v35"]["nuevos_indicadores"] == 22, "auditoría pública sincronizada"),
    ]
    payload = {
        "meta": {
        ...
        "checks": checks,
        "caveats": [
            "Las 67 cifras no forman una única cohorte y no se suman entre sí.",
