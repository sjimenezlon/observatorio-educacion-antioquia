(() => {
  ...
  function render(data) {
    ...
    const share = 100 * institution.matricula_2025_2 / data.sistema.matricula_2025_2;
    ...
    app.innerHTML = `${nav(iesId,data.instituciones)}
      <header class="hero"><div class="shell hero-grid"><div><span class="eyebrow">Perfil institucional · sistema público de Medellín</span><h1>${esc(institution.nombre_corto)}</h1><p class="hero-copy"><b style="color:var(--ink)">${esc(n.thesis)}.</b> ${esc(n.intro)}</p><div class="cut"><span>Matrícula institucional · 2025-2</span><span>Oferta comparable · SNIES 2024</span><span>Catálogo web · ${esc(institution.catalogo_web.consultado)}</span><span>Finanzas · cortes declarados</span></div><a class="official-site" href="${safeUrl(n.official)}" target="_blank" rel="noopener">Visitar sitio oficial de ${esc(institution.nombre_corto)} ↗</a></div><aside class="share-card"><small>Participación en la matrícula de las tres IES</small><strong class="share-value">${pct(share)}</strong><p>${fmt(institution.matricula_2025_2)} de ${fmt(data.sistema.matricula_2025_2)} estudiantes reportados conjuntamente por el Distrito.</p><div class="share-track" aria-hidden="true"><span style="width:${share}%"></span></div></aside></div></header>
      <main id="contenido" class="shell">
        <section class="kpis" aria-label="Cifras principales"><article class="kpi"><small>Matrícula 2025-2</small><strong>${fmt(institution.matricula_2025_2)}</strong><span>Cifra institucional publicada por el Distrito</span></article><article class="kpi"><small>Convocatoria 2026-2</small><strong>${announcedPrefix}${fmt(institution.programas_convocatoria_2026_2)}</strong><span>programas anunciados · ${cupsPrefix}${fmt(institution.cupos_2026_2)} cupos</span></article><article class="kpi"><small>Inventario SNIES 2024</small><strong>${fmt(snies.registros)}</strong><span>${fmt(snies.pregrado)} pregrados · ${fmt(snies.posgrado)} posgrados</span></article><article class="kpi"><small>Presupuesto 2026</small><strong>${money(finance.presupuesto_2026_millones)}</strong><span>Presupuesto aprobado, millones de pesos</span></article></section>
